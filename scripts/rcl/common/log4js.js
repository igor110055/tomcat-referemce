...
//         alert("in initialize this.inline[" + this.inline + "]");
        if(!this.inline) {
           if(!JsUtil.isGood(window.top.consoleWindow) )
           {
//              alert("window.top.consoleWindow is bad");
               window.top.consoleWindow = JsUtil.openNewWindow("", "log4jsconsole", "left=0,top=0,width=700,height=700,scrollbars=no,status=no,resizable=no;toolbar=no");
...
//		this.logElement.style.fontSize = "100%";
        this.logElement.style.fontFamily = "Courier, Arial, Helvetica, sans-serif";
	...
	output : function(message, style) {
		...
		this.outputCount++;
	  	style = (style ? style += ';' : '');
	  	style += 'padding:0px;margin:0 0 0px 0';
	...
	doAppend : function(loggingEvent) {
       ...
       //--- Nithy changes, addition check, suice we use only one global logger...
       if((!this.inline) && !(JsUtil.isGood(window.top.consoleWindow) && !window.top.consoleWindow.closed))
       {
//          alert("initialize 1");
          window.top.consoleWindow = null;
          this.initialize();
       }
       else
       {
          if(window.top.consoleWindow.opener != this.opener)
          {
//             alert("initialize 2");
             window.top.consoleWindow = null;
             this.initialize();
...
//todo: ConsoleAppender has a reference to logger?, cannot use same appender for other loggers.
//function getLogger(aCategory)
//{
//   var logger = window.top.rclLog;
//   if(JsUtil.isGood(window.top.rclLog))
//   {
//      logger = Log4js.getLogger(aCategory);
//      logger.setLevel(Log4js.Level.ALL);
//      logger.addAppender(window.top.rclConsoleAppender);
//   }
//
//   return logger;
//
...
* in following format:
* <pre>
* startTime [logLevel] message\n
* </pre>
*
* @constructor
* @extends Layout
*/
function BasicLayoutEx() {
	this.LINE_SEP  = "\n";
}
BasicLayoutEx.prototype = {
	/**
	 * Implement this method to create your own layout format.
	 * @param {Log4js.LoggingEvent} loggingEvent loggingEvent to format
	 * @return formatted String
	 ...
	 */
	format: function(loggingEvent) {
		return (this.formatTime(loggingEvent.startTime) + " [" + this.formatLevel(loggingEvent.level.toString()) + "] " + loggingEvent.message.escapeHTML() + this.LINE_SEP).fixed();
	},
	/**
	 * Returns the content type output by this layout.
	 * @return The base class returns "text/plain".
...
if(!JsUtil.isGood(window.top.rclLog))
{
   var rclLog = Log4js.getLogger("rcl");
   rclLog.setLevel(Log4js.Level.ALL);
//   alert("setting false for inline");
   var consoleAppender = new ConsoleAppender(rclLog, false);
   consoleAppender.setLayout(new BasicLayoutEx());
   rclLog.addAppender(consoleAppender);

   window.top.rclLog = rclLog;
   window.top.rclConsoleAppender = consoleAppender;
}
