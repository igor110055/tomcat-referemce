...
/**
 * Override the updateMenu method since Mobile wants to hide the drill down
 * menu item when the current selection doesn't support drill down
 */
DrillDownAction.prototype.updateMenu = function(jsonSpec) {
	var drillMgr = this.getCognosViewer().getDrillMgr();
	
	if(drillMgr && drillMgr.canDrillDown() == false) {
